                ...
                <div className="container">
                    <div className="row my-5">
                        <div className="col-lg-8 col-sm-12">
                            {
                            this.state.filter?this.state.filteredBlogs.map((blog)=><Blog key={blog._id} id={blog._id} genre={blog.genre} date={blog.date} author={blog.name} title={blog.title} desc={blog.content} email={blog.email} edit={blog.edit}/>)
                            :this.state.blogs.map((blog)=><Blog key={blog._id} id={blog._id} genre={blog.genre} date={blog.date} author={blog.name} title={blog.title} desc={blog.content} edit={blog.edit}/>)
                            }  
                        </div>
                        
                        <div className="col-lg-4 col-sm-12">
                            <div className="widget row" style={{textAlign:"center",display:"flex"}}>
                                <div className="col">
                                <button className="start_button" onClick={this.showModal}>Start Writing...</button>
                                
                                <button className="mt-5 btn-yellow" onClick={this.filterBlogsByUser}>Edit your articles</button>
                                </div>
                                <img src={pen} alt=""/>
                            </div>
